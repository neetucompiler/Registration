...
controller.hears('.*', ['direct_message', 'message_received', 'direct_mention', 'mention'], luisMiddleware.middleware.hereIntent, function (bot, message) {
  ...
  if (message.topIntent) {
    ...
    if (topIntent == 'Greeting') {
      ...
      global.sockets[message.raw_message.sender].emit('chat message', "Well, hello there!");
    } else if (topIntent == 'pageNotLoading') {
      global.sockets[message.raw_message.sender].emit('chat message', "Please follow these steps to recover PWA from not loading properly:<br><li>Go to Tools Menu in IE > Internet Options > Security > Trusted Sites > sites</li>");
      ...
      global.sockets[message.raw_message.sender].emit('chat message', '<a href="guide_images/1.1_scrn.png" target="_blank"><img src="guide_images/1.1_scrn.png" width="225" height="200"></a>');
    } else if (topIntent == 'cantEditActionPlan') {
      global.sockets[message.raw_message.sender].emit('chat message', "This might resolve your issue.");
      ...
      global.sockets[message.raw_message.sender].emit('chat message', "If the BD/BDM has done all these steps and still you (Supply Planner / Production Engineer) can’t edit the Actions, then contact your APL facilitator to check your access rights in the tool.");
    } else if (topIntent == 'actionPlanFollowUp') {
      global.sockets[message.raw_message.sender].emit('chat message', "The tool is not available for Suppliers now (it will happen in the future). But for now the KPIs and Actions can be exported as pdf from reports and shown to the supplier.");
    } else if (topIntent == 'BusinessDevSupplyPlannerDifference') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Business Developers and Business Development Managers have write access and Supply Planners and Production Engineers edit access.')
      ...
      global.sockets[message.raw_message.sender].emit('chat message', 'Supply Planners and Production Engineers <li>Edit access</li><li>Register/Edit KPIs</li><li>Register/Edit Actions</li><li>Register/Edit SWOT</li><li>Register/Edit SWOT</li><li>Register/Edit Risks</li>Only for the Supplier Action Plans they have been added through the Build Team functionality');
    } else if (topIntent == 'KPIReport') {
      global.sockets[message.raw_message.sender].emit('chat message', 'In the reports you go to Sup View and you can add as filter either the BD name or the supplier numbers (all supplier numbers that a BD has) and that is how you see the KPIs for all the suppliers belonging to one BD. Same for Actions.');
    } else if (topIntent == 'multipleYearsAction') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Actions should have the start and end date during the Financial Year, and in these cases the action needs to be created again when a new Action Plan is created.');
    } else if (topIntent == 'maxCharacters') {
      global.sockets[message.raw_message.sender].emit('chat message', '255 Characters.');
    } else if (topIntent == 'KPIGoalSettingDecimals') {
      global.sockets[message.raw_message.sender].emit('chat message', 'The system allows 2 decimals for all the KPI cells.');
    } else if (topIntent == 'CheckInCheckOut') {
      global.sockets[message.raw_message.sender].emit('chat message', 'When you create a new Action Plan you are the owner of that project and automatically, the Action Plan is Checked out to you (this means that you can edit it).<br><br>An Action Plan can be checked out to someone (or to you) for editing and this means that it is locked by that person (or you) for editing.');
      ...
      global.sockets[message.raw_message.sender].emit('chat message', 'When you access the project again in order to check it out for editing go to the top menu / Project tab / Edit icon.');
    } else if (topIntent == 'checkpointStatus') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Checkpoint status is a functionality that can be used during the creation phase to check the KPIs on Business Navigation level. The field is optional.<br><br>For example: if the agreement is to have 2 check points before submitting to the next phase, select the 2 Checkpoints row in that box. When you are ready with your KPIs to be checked for the first time during the Creation Phase select the Ready for Checkpoint 1 (the 4th row in this example). And then for the second revision the 5th row in this case: Ready for checkpoint 2.');
      global.sockets[message.raw_message.sender].emit('chat message', '<a href="guide_images/20_scrn.png" target="_blank"><img src="guide_images/20_scrn.png" width="225" height="200"></a>');
    } else if (topIntent == 'seeActionPlanWhenStatusClosed') {
      global.sockets[message.raw_message.sender].emit('chat message', 'When you open a Closed Action Plan and go to Actions you will land on this page. To view the Actions click on Actions Details.');
      ...
      global.sockets[message.raw_message.sender].emit('chat message', '<a href="guide_images/11.1_scrn.png" target="_blank"><img src="guide_images/11.1_scrn.png" width="225" height="200"></a>');
    } else if (topIntent == 'cantPasteTextInAction') {
      global.sockets[message.raw_message.sender].emit('chat message', 'This functionality is not available in this version of the tool.');
    } else if (topIntent == 'warpText') {
      global.sockets[message.raw_message.sender].emit('chat message', 'This functionality is not available in this version of the tool.');
    } else if (topIntent == 'changeActionPlanOwner') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Yes, the Facilitator / Super User can change the owner of the APL at any stage. After the owner is changed and if he/she is responsible for some actions you need to add his name also in the Build team and then change the Action Responsible in the Actions page.');
    } else if (topIntent == 'changeActionResponsible') {
      global.sockets[message.raw_message.sender].emit('chat message', 'During the year you can change the members working on an Action Plan from Build Team. There users can be added or removed at any stage of the Action Plan (except when the Action Plan is in status closed).');
      ...
      global.sockets[message.raw_message.sender].emit('chat message', 'The information about the Business Team Members shown on the Supplier Info page of the Action Plan is not related to the Build Team. That information comes from the Global KPI report and the data from that report is being updated almost every month.');
    } else if (topIntent == 'OTDSenderDirectSupplyNotShown') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Pay attention that the value entered in the las cell of each section is registered (click Tab, Enter or move to another cell and make sure the value is in the right format). If it doesn’t show in the right format it will not be saved.');
      global.sockets[message.raw_message.sender].emit('chat message', '<a href="guide_images/16_scrn.png" target="_blank"><img src="guide_images/16_scrn.png" width="225" height="200"></a>');
    } else if (topIntent == 'addUsersToBuildTeam') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Yes, the Build Team functionality is per each Supplier Action Plan (not for all the suppliers under the same BDM).');

    } else if (topIntent == 'deleteActions') {
      global.sockets[message.raw_message.sender].emit('chat message', 'Yes, actions can be deleted no matter the stage of and Action Plan (Creation, Forecast or Follow-up). And actions can be deleted by all users that have write or edit access to the tool.<br><br><br><br>Instead of deleting Actions during the Financial Year we recommend to use the status Cancelled in Action Status field and to add comments on the reason the Action was cancelled. But if there is a need to delete an Action then this can be done from the Top menu / Task tab / Delete Action.');
      global.sockets[message.raw_message.sender].emit('chat message', '<a href="guide_images/18_scrn.png" target="_blank"><img src="guide_images/18_scrn.png" width="225" height="200"></a>');
    } else if (topIntent == 'None') {
      //global.sockets[message.raw_message.sender].emit('chat message', "You have reached the greeting intent, you said "+session.message.text);
      global.sockets[message.raw_message.sender].emit('chat message', "I could not understand what you just said. Please reword your statement and try again.");

    } else if (topIntent == 'Help') {
      //global.sockets[message.raw_message.sender].emit('chat message', "You have reached the greeting intent, you said "+session.message.text);
      global.sockets[message.raw_message.sender].emit('chat message', "I am Ikea bot and I can help clear your doubts on any topic related to the dunctions in the company. Please feel free to ask anything that you like.");
      global.sockets[message.raw_message.sender].emit('chat message', "You can ask stuff like \"What is the difference between a supply manager and a business developer?\"; or<br>\"How can I delete actions?\";or<br>\"What is the maximum number of characters in the action descriptioon field?\"; or anything else.");
